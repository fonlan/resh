...
| `src/hooks/useTerminal.ts` | macOS 注入 + composition 跟踪 + 40ms onData 去重 |
...
- Phase 1 调试开关已在 Phase 3 移除

## 验证（本阶段）

- `assertMacOsImeShiftSymbolSelfCheck()` 模块加载时自检  
- `npm run typecheck`  
- 真实 macOS 中文 IME 打键：见 `notes/macos/phase-3-ime-shift-symbol-verify.md`
