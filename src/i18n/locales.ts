export const locales = {
  en: {
    settings: "Settings",
    sync: "Sync",
    servers: "Servers",
    auth: "Authentication",
    proxies: "Proxies",
    general: "General",
    routing: "Routing",
    advanced: "Advanced",
    snippets: "Snippets",
    appearance: "Appearance",
    theme: "Theme",
    language: "Language",
    system: "System",
    light: "Light",
    dark: "Dark",
    orange: "Sunset Orange",
    green: "Sage Forest",
    cursorStyles: {
      block: "Block",
      underline: "Underline",
      bar: "Bar",
    },
    terminal: "Terminal",
    fontFamily: "Font Family",
    fontSize: "Font Size",
    cursorStyle: "Cursor Style",
    fontFamilyPlaceholder: "e.g., 'Courier New', monospace",
    scrollback: "Scrollback Lines",
    webdav: "WebDAV Sync",
    webdavUrl: "WebDAV URL",
    webdavProxy: "Proxy Server",
    syncNow: "Sync Now",
    syncing: "Syncing...",
    syncSuccess: "Sync successful",
    syncFailed: "Sync failed",
    syncConflictsTitle: "Resolve sync conflicts",
    syncConflictsDescription:
      "Choose which complete version to keep for every conflicting item. Nothing is changed until you apply these choices.",
    syncConflictsApply: "Apply choices",
    syncConflictsPending: "{count} sync item(s) need your resolution.",
    syncConflictsReview: "Review conflicts",
    syncConflictsKeepAllLocal: "Keep all local",
    syncConflictsUseAllRemote: "Use all remote",
    syncConflictsSelected: "Selected {selected} of {total}",
    syncConflictChooseAll: "Choose a local or remote version for every conflict.",
    syncConflictEntity: "Item type",
    syncConflictKeepLocal: "Keep local",
    syncConflictUseRemote: "Use remote",
    syncConflictDeleted: "Deleted",
    syncConflictsRefreshRequired:
      "This conflict attempt is no longer current. Refresh synchronization and try again.",
    username: "Username",
    password: "Password",
    recordingMode: "Recording Mode",
    recordingModes: {
      raw: "Raw (Includes ANSI codes)",
      text: "Text (Strip ANSI codes)",
    },
    confirmations: "Confirmations",
    confirmCloseTab: "Confirm before closing tabs",
    confirmExitApp: "Confirm before exiting application",
    debugEnabled: "Enable debug logs",
    maxRecentServers: "Max recent connections (Welcome page)",
    tabWidthMode: "Tab Width Mode",
    tabWidthModes: {
      adaptive: "Adaptive (by title)",
      fixed: "Fixed",
    },
    fixedTabWidth: "Fixed Tab Width (px)",
    fixedTabWidthHint: "Range: {min}-{max} px",
    terminalRightClickMode: "Terminal Right-Click Mode",
    terminalRightClickModes: {
      contextMenu: "Context Menu (Copy/Paste)",
      selectionCopyPaste: "Select to Copy + Right-Click to Paste",
    },
    terminalCommandBlockBar: "Show command block color bars",
    terminalRenderer: "Terminal Renderer",
    terminalRenderers: {
      canvas: "Canvas (Default, stable)",
      webgl: "WebGL (Faster, may leave artifacts)",
    },
    terminalRendererHint:
      "WebGL is faster but can leave stale artifacts on screen with full-screen TUI apps. Canvas is the default and most stable.",
    tabNewServerSort: "New tab server list sort",
    tabNewServerSorts: {
      default: "Default sort (grouped)",
      recent: "Recently connected",
      connectionCount: "Most connected",
      createdAt: "Created time",
      updatedAt: "Updated time",
    },
    softwareUpdate: "Software Update",
    updateAutoCheck: "Automatically check for updates",
    updateAutoCheckHint:
      "When enabled, Resh checks GitHub for stable releases after startup and every 6 hours. Manual checks from About still work when this is off.",
    updateProxy: "Update proxy",
    updateProxyHint:
      "Used for update checks and downloads. Does not affect SSH connections.",
    updateProxyMissing:
      "The previously selected proxy was removed. Direct connection will be used until you pick another proxy.",
    updateStatusChecking: "Checking for updates…",
    updateStatusUpToDate: "You're up to date",
    updateStatusAvailable: "Update available: {version}",
    updateStatusDownloading: "Downloading update…",
    updateStatusInstalling: "Installing update…",
    updateStatusReady: "Update downloaded and verified",
    updateStatusWaitingRestart: "Waiting to restart safely…",
    updateStatusError: "Update check failed",
    updateStatusRateLimited: "GitHub rate limit reached. Try again later.",
    updatePhaseChecksums: "Downloading checksums…",
    updatePhaseVerifying: "Verifying package…",
    updateCheckNow: "Check for updates",
    updateDownload: "Download",
    updateDownloadInstall: "Download and install",
    updateInstallLater: "Close",
    updateOpenDialog: "View update details",
    updateCurrentVersion: "Current version",
    updateTargetVersion: "New version",
    updateDownloadSize: "Download size",
    updateRestartConfirmHint:
      "Resh will disconnect open SSH sessions, wait for safe write operations to finish, then restart and restore your tabs. Active connections will not stay open across the restart.",
    updateRestartLaterHint: "Keep the verified package and close this dialog",
    updateRestartNow: "Restart and update",
    updateRestartWaiting: "Waiting for safe operations to finish…",
    updateRestartBlockedDirty:
      "Save or discard unsaved editor tabs before restarting.",
    updateRestartBlockedSaving: "Wait for editor saves to finish before restarting.",
    updateRestartBlockedRecording:
      "Stop terminal recording and wait for it to flush before restarting.",
    updateRestartBlockedSettings:
      "Finish or close settings (pending save) before restarting.",
    updateRestartCancelWait: "Cancel restart",
    updateRestartKeepWaiting: "Keep waiting",
    updateRestartTimeout:
      "Some operations are still running. You can keep waiting or cancel restart.",
    updateRestartCategoryConfig: "Saving configuration",
    updateRestartCategorySync: "WebDAV sync",
    updateRestartCategoryTransfer: "SFTP transfers",
    updateRestartCategoryEditUpload: "SFTP editor uploads",
    updateRestorePartial:
      "Restored tabs with some failures: {details}",
    updateRestoreSuccess: "Restored previous session after update",
    updateInstallFailed: "Update install failed: {details}",
    updateInstallSuccess: "Updated to version {version}",
    updateTitleButton: "Update available",
    updateTitleDownloading: "Downloading update",
    updateTitleReady: "Update ready",
    updateTitleError: "Update error",
    updateErrorNetwork: "Network error while checking for updates",
    updateErrorIncomplete: "Release is missing required install files",
    updateErrorProxy: "Update proxy is invalid or unavailable",
    saveStatus: {
      saving: "Saving...",
      saved: "Saved",
      error: "Error saving",
    },
    common: {
      add: "Add",
      edit: "Edit",
      delete: "Delete",
      save: "Save",
      cancel: "Cancel",
      name: "Name",
      path: "Path",
      host: "Host",
      port: "Port",
      type: "Type",
      none: "None",
      proxy: "Proxy",
      noProxy: "No Proxy (Direct Connection)",
      remove: "Remove",
      rename: "Rename",
      create: "Create",
      apply: "Apply",
      loading: "Loading configuration...",
      connect: "Connect",
      actions: "Actions",
      optional: "Optional",
      syncThisItem: "Sync this item",
      enableSync: "Enable Sync",
      queued: "Queued",
    },
    serverTab: {
      title: "SSH Servers",
      addServer: "Add Server",
      editServer: "Edit Server",
      emptyState: "No servers configured. Add one to get started.",
      defaultGroup: "Default Group",
      connectTooltip: "Connect to server",
      editTooltip: "Edit server",
      deleteTooltip: "Delete server",
      deleteConfirmation: "Are you sure you want to delete this server?",
      deleteInUseConfirmation:
        "This server is currently being used as a jumphost by one or more servers. Are you sure you want to delete it? The affected servers will have their jumphost settings cleared.",
      jumphost: "Jumphost",
    },
    authTab: {
      title: "SSH Authentications",
      addAuth: "Add Authentication",
      editAuth: "Edit Authentication",
      emptyState: "No authentications configured. Add one to get started.",
      editTooltip: "Edit authentication",
      deleteTooltip: "Delete authentication",
      deleteInUseConfirmation:
        "This authentication is currently in use by one or more servers. Are you sure you want to delete it? The affected servers will have their authentication set to None.",
      passwordType: "Password",
      keyType: "SSH Key",
    },
    proxyTab: {
      title: "Proxy Servers",
      addProxy: "Add Proxy",
      editProxy: "Edit Proxy",
      emptyState: "No proxies configured. Add one to get started.",
      editTooltip: "Edit proxy",
      deleteTooltip: "Delete proxy",
      deleteInUseConfirmation:
        "This proxy is currently in use by one or more servers. Are you sure you want to delete it? The affected servers will have their proxy settings cleared.",
    },
    snippetsTab: {
      title: "Code Snippets",
      addSnippet: "Add Snippet",
      editSnippet: "Edit Snippet",
      emptyState: "No snippets configured. Add one to get started.",
      editTooltip: "Edit snippet",
      deleteTooltip: "Delete snippet",
      deleteConfirmation: "Are you sure you want to delete this snippet?",
    },
    serverForm: {
      nameLabel: "Server Name",
      namePlaceholder: "e.g., Production Server, AWS EC2",
      groupLabel: "Group",
      groupPlaceholder: "e.g., Production, Staging",
      hostPlaceholder: "e.g., example.com or 192.168.1.100",
      usernameLabel: "Username",
      usernamePlaceholder: "e.g., ubuntu",
      authLabel: "Authentication",
      authPlaceholder: "Select authentication...",
      routingTitle: "Connection Routing (Optional)",
      proxyLabel: "Proxy",
      jumphostLabel: "Jumphost (SSH Tunnel)",
      portForwardingTitle: "Port Forwarding",
      localPortPlaceholder: "Local port",
      remotePortPlaceholder: "Remote port",
      keepaliveTitle: "Keepalive Settings",
      keepaliveInterval: "Keepalive Interval (seconds, 0 = disabled)",
      autoExecTitle: "Auto-Execute Commands",
      addCommand: "Add Command",
      autoExecPlaceholder: "e.g., cd /var/log",
      favoritesTab: "Favorites",
      sftpFavoritePathsTitle: "SFTP Favorite Paths",
      sftpFavoritePathsHint:
        "These paths are saved under this server and used by the SFTP sidebar favorites menu.",
      sftpFavoritePathPlaceholder: "e.g., /var/log/nginx",
      sftpFavoritePathsEmpty: "No favorite paths configured for this server.",
      addFavoritePath: "Add",
      envVarsTitle: "Environment Variables",
      varNamePlaceholder: "Variable name",
      varValuePlaceholder: "Value",
    },
    authForm: {
      nameLabel: "Authentication Name",
      namePlaceholder: "e.g., My AWS Key, Work Password",
      typeLabel: "Authentication Type",
      passwordLabel: "Password",
      passwordPlaceholder: "Enter password",
      keyContentLabel: "SSH Key Content",
      uploadKey: "Upload Key File",
      keyContentPlaceholder:
        "Paste your SSH private key here (-----BEGIN PRIVATE KEY-----...)",
      passphraseLabel: "Key Passphrase (Optional)",
      passphrasePlaceholder: "Enter passphrase if your key is encrypted",
    },
    proxyForm: {
      nameLabel: "Proxy Name",
      namePlaceholder: "e.g., Corporate Proxy, Home SOCKS5",
      typeLabel: "Proxy Type",
      hostLabel: "Host",
      hostPlaceholder: "e.g., proxy.example.com",
      portLabel: "Port",
      authTitle: "Authentication (Optional)",
      usernamePlaceholder: "Leave empty if no auth required",
      passwordPlaceholder: "Leave empty if no auth required",
      ignoreSslLabel: "Ignore SSL Certificate Errors",
      ignoreSslDesc: "Enable for corporate proxies with MITM inspection",
    },
    snippetForm: {
      nameLabel: "Name",
      namePlaceholder: "Snippet Name",
      contentLabel: "Content (Bash/Command)",
      contentPlaceholder: "#!/bin/bash...",
      descriptionLabel: "Description (Optional)",
      descriptionPlaceholder: "Brief description of what this does",
      groupLabel: "Group",
      groupPlaceholder: "Group Name",
      defaultGroup: "Default",
    },
    manualAuth: {
      title: "Manual Authentication",
      retryTitle: "Authentication Failed",
      retryDescription: "Authentication failed for {server}. Please try again:",
      enterCredentials: "Enter credentials for {server}:",
      usernameLabel: "Username",
      passwordLabel: "Password",
      passwordPlaceholder: "Leave empty if using key",
      orDivider: "— OR —",
      privateKeyLabel: "Private Key (PEM)",
      privateKeyPlaceholder: "Paste your private key here...",
      rememberCredentials: "Remember authentication information",
    },
    welcome: {
      title: "Welcome to Resh",
      subtitle: "A professional, high-performance SSH client.",
      recentTitle: "Recent Connections",
      viewAll: "View All",
      newConnection: "New Connection",
      configureNew: "Configure a new server",
      noServers: "No servers configured",
      getFirstStarted: "Add your first server to get started with Resh.",
    },
    mainWindow: {
      closeTab: "Close tab",
      cloneTab: "Clone tab",
      closeOthers: "Close other tabs",
      reconnect: "Reconnect",
      settings: "Settings",
      snippets: "Snippets",
      aiAssistant: "AI Assistant",
      snippetsTooltip: "Code Snippets",
      exportLogs: "Export session",
      startRecording: "Start Recording",
      stopRecording: "Stop Recording",
      tabAriaLabel: "Tab {index} of {total}",
      splitView: "Split view",
      exitSplitView: "Exit split view",
      splitLeftRight: "Left and right split",
      splitTopBottom: "Top and bottom split",
      splitFourGrid: "Four-pane grid",
      splitSelectTabsTitle: "Choose tabs for split view",
      splitSelectTabsHint: "Select {count} tabs for {layout}",
      splitSelectedCount: "{selected}/{required} selected",
      editorUnsavedTitle: "Unsaved changes",
      editorUnsavedSwitch:
        '"{name}" has unsaved changes. Save before switching tabs?',
      editorUnsavedClose:
        '"{name}" has unsaved changes. Save before closing this tab?',
      discardChanges: "Discard",
      editor: {
        invalidPayload: "Invalid editor payload.",
        documentMissing: "Editor document not found.",
        saveFailed: "Failed to save editor file: {error}",
      },
      editorConflict: {
        title: "Remote file changed",
        description:
          "The remote file changed while you were editing {path}. Your local edits have been kept and were not saved.",
        remoteChanged: "The remote file metadata changed.",
        remoteDeleted: "The remote file was deleted.",
        remotePath: "Remote path",
        remoteSize: "Remote size",
        remoteModified: "Remote modified",
        notAvailable: "Not available",
        snapshotUnavailable: "The current remote text could not be loaded: {error}",
        snapshotUnavailableShort: "The current remote text is unavailable.",
        localVersion: "Your local edits",
        remoteVersion: "Current remote version",
        viewDiff: "View diff",
        hideDiff: "Hide diff",
        adoptRemote: "Use remote version",
        overwriteRemote: "Overwrite remote",
        overwriteConfirm:
          "This will replace the current remote file with your local edits. This cannot be undone.",
        overwriteConfirmAction: "Overwrite remote now",
        adopted: "Replaced the editor contents with the current remote version.",
        conflictStillCurrent:
          "The remote file changed again. Review the latest version before overwriting.",
      },
      externalEditConflict: {
        title: "External edit auto-upload paused",
        description:
          "Automatic upload for {path} is paused because the remote file changed while you edited it locally.",
        autoUploadPaused: "Automatic upload is paused for this file.",
        remoteChanged: "The remote file metadata changed.",
        remoteDeleted: "The remote file was deleted.",
        pendingLocal:
          "Your latest local save is kept as the pending version to upload.",
        remotePath: "Remote path",
        localPath: "Local temp file",
        remoteSize: "Remote size",
        remoteModified: "Remote modified",
        notAvailable: "Not available",
        snapshotUnavailable:
          "The current remote revision could not be fully loaded: {error}",
        changedHint:
          "Use the remote version to replace the local temp file, or overwrite remote with your local file. Keep paused leaves auto-upload off until you decide.",
        deletedHint:
          "The remote file no longer exists. Keep paused, or explicitly recreate it from your local temp file.",
        keepPaused: "Keep paused",
        adoptRemote: "Use remote version",
        overwriteRemote: "Overwrite remote",
        recreateRemote: "Recreate remote file",
        overwriteConfirm:
          "This will replace the current remote file with your local temp file. This cannot be undone.",
        overwriteConfirmAction: "Overwrite remote now",
        recreateConfirm:
          "This will create the remote file again from your local temp file.",
        recreateConfirmAction: "Recreate remote now",
        adopted:
          "Replaced the local temp file with the remote version. Auto-upload resumed.",
        overwrote: "Uploaded the local temp file and resumed auto-upload.",
        keptPaused: "Auto-upload stays paused for this file.",
        conflictStillCurrent:
          "The remote file changed again. Review the latest conflict before overwriting.",
        resolveFailed: "Failed to resolve external edit conflict: {error}",
        bannerTitle: "External edit upload paused",
        bannerAction: "Review",
      },
    },
    windowControls: {
      minimize: "Minimize",
      maximize: "Maximize",
      close: "Close",
    },
    terminalTab: {
      connecting: "Connecting to server {name}...",
      connected: "Connected! Session ID: {id}",
      connectionClosed: "Connection closed.",
      error: "Error: {error}",
      hostCopied: "Host copied to clipboard",
      contextMenuCopied: "Copied to clipboard",
      contextMenuCopy: "Copy",
      contextMenuPaste: "Paste",
    },
    editorTab: {
      undo: "Undo",
      redo: "Redo",
      save: "Save",
      wordWrap: "Wrap",
      wordWrapEnable: "Enable word wrap",
      wordWrapDisable: "Disable word wrap",
      language: "Document Type",
      encoding: "Encoding: {encoding}",
      dirty: "Modified",
      saved: "Saved",
      remoteChanged: "Remote changed",
      lineColumn: "Ln {line}, Col {column}",
    },
    newTabButton: {
      connectTo: "Connect to Server",
      quickConnectPlaceholder: "Quick connect: ip, host or user@host",
      quickConnectAction: "Quick Connect",
      quickConnectInvalid: "Use ip, host, or user@host",
      manageServers: "Manage Servers",
      collapseGroup: "Collapse group",
      expandGroup: "Expand group",
    },
    ai: {
      sidebarTitle: "AI Assistant",
      tabTitle: "AI",
      configuration: "Configuration",
      newChat: "New Chat",
      history: "History",
      clearHistory: "Clear History",
      typeMessage: "Type a message...",
      thinkingProcess: "Thinking Process",
      stopGeneration: "Stop Generation",
      startChat: "Start Chat",
      copyMessage: "Copy Message",
      regenerateMessage: "Regenerate Response",
      requestFailed: "AI request failed: {error}",
      unknownError: "Unknown error",
      noHistory: "No history available",
      deleteSessionConfirm: "Are you sure you want to delete this session?",
      clearHistoryConfirm:
        "Are you sure you want to clear all history for this server?",
      selectServer: "Select a server and start a new chat to begin.",
      editorContext: {
        includeCurrentFile: "Include current editor file",
        fileLabel: "File: {path}",
        languageLabel: "Lang: {language}",
        charCountLabel: "Chars: {count}",
        readOnlyHint: "Sent as read-only context for analysis.",
        tooLarge:
          "Current editor content has {count} chars, exceeding limit {max}. Turn off this option or reduce file size.",
      },
      globalAdditionalPrompt: "Global Additional Prompt",
      globalAdditionalPromptDesc:
        "This prompt will be appended to the system prompt for all AI requests. Syncs across devices.",
      globalAdditionalPromptPlaceholder:
        "e.g. Always answer in Chinese. Be concise.",
      serverAdditionalPrompt: "Server Additional Prompt",
      serverAdditionalPromptDesc:
        "This prompt is specific to this server and will be appended after the global prompt.",
      serverAdditionalPromptPlaceholder:
        "e.g. This server runs RHEL 8 with Podman. Prefer podman commands.",
      maxChatContext: "Max Chat Context",
      requestTimeout: "Request Timeout (seconds)",
      toolConfirmationCountdown: "Tool confirmation countdown (seconds)",
      toolConfirmationCountdownYoloWarning:
        "0 = YOLO mode: all tool calls (including dangerous tools) execute without confirmation. High-risk commands (rm, dd, mkfs, fdisk, reboot, shutdown, halt, poweroff, init) still require confirmation.",
      yoloModeBadge: "YOLO mode active - no tool confirmations",
      channels: "AI Channels",
      models: "AI Models",
      addChannel: "Add Channel",
      addModel: "Add Model",
      noChannels: "No AI Channels configured.",
      noModels: "No AI Models configured.",
      editChannel: "Edit AI Channel",
      editModel: "Edit AI Model",
      deleteChannelConfirm:
        "Are you sure you want to delete this AI Channel? Associated models will be affected.",
      deleteModelConfirm: "Are you sure you want to delete this AI Model?",
      disabled: "Disabled",
      channelDisabled: "Channel Disabled",
      channelForm: {
        name: "Name",
        namePlaceholder: "e.g. OpenAI Personal",
        type: "Type",
        endpoint: "Endpoint (Optional)",
        apiKey: "API Key (Optional)",
        active: "Enable this channel",
      },
      modelForm: {
        name: "Model Name",
        namePlaceholder: "e.g. gpt-4, gpt-3.5-turbo",
        channel: "Channel",
        active: "Enable this model",
        contextWindow: "Context Window (tokens)",
        responseReserve: "Response Reserve (tokens)",
      },
      modelCatalog: "Model catalog (models.dev)",
      catalogModels: "models",
      catalogProviders: "providers",
      catalogFresh: "fresh",
      catalogStale: "stale",
      catalogFetched: "Last updated",
      catalogNextRefresh: "Next auto refresh",
      catalogLoading: "Loading catalog status…",
      catalogRefresh: "Update catalog now",
      catalogRefreshing: "Updating…",
      catalogRefreshOk: "Catalog updated",
      catalogRefreshFailed: "Catalog update failed",
      catalogLookupBusy: "Looking up models.dev catalog…",
      catalogLookupError: "Catalog lookup failed",
      catalogLookupMatch: "Fetched from the models.dev catalog",
      catalogLookupContext: "Max context",
      catalogLookupOutput: "Output reserve",
      catalogLookupNone: "Not found in the models.dev catalog — fill in manually",
      catalogLookupDuplicates: "This id exists under {count} providers; preferred:",

      tool: {
        confirmExecution: "Confirm Execution",
        autoExecute: "Auto-execute in {seconds}s",
        cancel: "Cancel",
        decline: "Decline",
        allowOnce: "Allow Once",
        allowForSession: "Allow for Session",
        cancelRun: "Cancel Run",
        resolving: "Waiting for confirmation…",
        runNow: "Run Now ({seconds}s)",
        confirmRun: "Confirm Run",
        usingTools: "Using tools: {tools}",
        codeCopied: "Code copied",
        copyCode: "Copy code",
        insertToTerminal: "Insert to terminal",
        executeCommand: "Execute Command",
        executeBackgroundCommand: "Execute Background Command",
        getTerminalOutput: "Read Terminal Output",
        getSelectedTerminalOutput: "Read Selected Terminal Output",
        readFile: "Read Remote File",
        sendInterrupt: "Send Interrupt (Ctrl+C)",
        sendTerminalInput: "Send Terminal Input",
        sftpDownload: "SFTP Download",
        sftpUpload: "SFTP Upload",
        timeoutSeconds: "Timeout: {seconds}s",
        waitFinish: "Wait for completion: {value}",
        waitFinishOn: "Yes",
        waitFinishOff: "No",
        commandOutput: "Command Output",
        noCommandOutput: "No captured output",
      },
    },
    sftp: {
      title: "SFTP",
      transfers: "Transfers",
      noFiles: "No files",
      notConnected: "Not connected",
      loading: "Loading...",
      sort: {
        name: "Name",
        dateModified: "Date Modified",
      },
      contextMenu: {
        open: "Open",
        download: "Download",
        upload: "Upload",
        newFile: "New File",
        newFolder: "New Folder",
        properties: "Properties",
        rename: "Rename",
        delete: "Delete",
        path: "Path",
        copyFileName: "Copy File Name",
        copyFolderName: "Copy Folder Name",
        copyFullPath: "Copy Full Path",
        sendPathToTerminal: "Send Path to Terminal",
        terminalJump: "Terminal Jump to This Path",
        edit: "Edit",
        editServerVim: "Open with Server Vim",
        editLocal: "Open with Local Editor",
        copy: "Copy",
        cut: "Cut",
        pasteCopy: "Paste Copy",
        pasteMove: "Paste Move",
        cancel: "Cancel",
        commands: "Commands",
        addFavorite: "Add to Favorites",
      },
      tooltips: {
        sort: "Sort",
        favorites: "Favorites",
        refresh: "Refresh",
        lock: "Lock Sidebar",
        unlock: "Unlock Sidebar",
        close: "Close",
      },
      favorites: {
        empty: "No favorite directories yet",
        remove: "Remove favorite path",
        pathNotFound: "Favorite path no longer exists: {path}",
      },
      editor: {
        serverUnavailable: "Server is unavailable for this file.",
        nonTextFile: "This file is not text and cannot be opened in editor.",
        openFailed: "Failed to open file in editor: {error}",
        unknownError: "Unknown error",
      },
      modals: {
        deleteConfirmTitle: "Delete Confirmation",
        deleteConfirmMessage:
          "Are you sure you want to delete this item? This action cannot be undone.",
        copyFallbackTitle: "Use Terminal Copy?",
        copyFallbackMessage:
          "Server-side copy is not supported. Use cp in the associated terminal? If not, we will fallback to client streaming copy.",
        copyFallbackUseCp: "Use cp Command",
        copyFallbackUseStreaming: "Use Streaming Copy",
        newFileTitle: "New File",
        newFolderTitle: "New Folder",
        renameTitle: "Rename Item",
        propertiesTitle: "Properties",
        permissionsLabel: "Permissions (Octal)",
        permissionsPlaceholder: "e.g., 755",
        itemNameLabel: "Name",
      },
      settings: {
        defaultDownloadPath: "Default Download Path",
        defaultDownloadPathPlaceholder: "e.g. C:\\Users\\User\\Downloads",
        defaultDownloadPathDesc: "Default directory for SFTP file downloads.",
        maxConcurrentTransfers: "Max Concurrent Transfers",
        maxConcurrentTransfersDesc:
          "Number of simultaneous download/upload tasks. Recommended: 2-4.",
        maxConcurrentTransfersPerSession: "Max Concurrent Per Session",
        maxConcurrentTransfersPerSessionDesc:
          "Upper bound for active transfer tasks per SSH session, used for queue fairness.",
        transferProfile: "Transfer Profile",
        transferProfileDesc:
          "Balanced is recommended. Safe reduces risk on unstable links, Fast favors throughput.",
        profileSafe: "Safe (Conservative)",
        profileBalanced: "Balanced (Recommended)",
        profileFast: "Fast (Aggressive)",
        downloadMaxInflight: "Download Max In-Flight",
        downloadMaxInflightDesc:
          "Upper bound for pipelined read requests (future download pipeline phase).",
        uploadMaxInflight: "Upload Max In-Flight",
        uploadMaxInflightDesc:
          "Upper bound for concurrent chunk writes in a single upload task.",
        chunkSizeMinKb: "Chunk Size Min (KB)",
        chunkSizeMaxKb: "Chunk Size Max (KB)",
        chunkSizeDesc:
          "Chunk size range used by transfer tuning. Effective size is bounded by server limits.",
        enableMultiConnectionForSmallFiles:
          "Enable multi-connection mode for many small files",
        enableLargeFileStriping:
          "Enable large-file striping mode (experimental)",
        experimentalTuningDesc:
          "Experimental switches are off by default and may change in future releases.",
        browse: "Select Folder",
        editorAssociations: "Editor Associations",
        addRule: "Add Rule",
        filePattern: "File Pattern",
        editorPath: "Editor Path",
        browseFile: "Select Editor",
        noRules:
          "No editor rules defined. Add one to associate file types with local editors.",
        patternPlaceholder: "*.log",
        editorPlaceholder: "C:\\Windows\\notepad.exe",
        priorityHint:
          "Tip: Drag rules to reorder. Rules at the top have higher priority.",
        customCommands: "Custom Commands",
        addCommand: "Add Command",
        commandSync: "Sync",
        commandName: "Name",
        commandPattern: "Pattern",
        commandExec: "Command",
        commandPlaceholderName: "Name",
        commandPlaceholderPattern: "*.log",
        commandPlaceholderExec: "tail -f {fpath}",
        commandHelp:
          "Use {fpath} for absolute file path, {dpath} for directory path, and {fname} for file name without extension.",
        noCommands: "No custom commands defined",
        synced: "Synced",
        localOnly: "Local Only",
      },
    },
    about: {
      tabTitle: "About",
      description:
        "A modern, secure multi-tab SSH client built with Tauri 2 + React 19.",
      version: "Version",
      author: "Author",
      github: "GitHub Repository",
      license: "License",
      techStack: "Tech Stack",
      thanks:
        "Special thanks to all contributors and the open source community.",
    },
  },
  "zh-CN": {
    settings: "设置",
    sync: "同步",
    servers: "服务器",
    auth: "身份验证",
    proxies: "代理设置",
    general: "常规",
    routing: "路由",
    advanced: "高级",
    snippets: "代码段",
    appearance: "外观",
    theme: "主题",
    language: "语言",
    system: "跟随系统",
    light: "浅色",
    dark: "深色",
    orange: "落日橙",
    green: "鼠尾草",
    cursorStyles: {
      block: "方块",
      underline: "下划线",
      bar: "竖杠",
    },
    terminal: "终端",
    fontFamily: "字体名称",
    fontSize: "字体大小",
    cursorStyle: "光标样式",
    fontFamilyPlaceholder: "例如：'Courier New', monospace",
    scrollback: "滚动回溯行数",
    webdav: "WebDAV 同步",
    webdavUrl: "WebDAV 地址",
    webdavProxy: "代理服务器",
    syncNow: "立即同步",
    syncing: "同步中...",
    syncSuccess: "同步成功",
    syncFailed: "同步失败",
    syncConflictsTitle: "解决同步冲突",
    syncConflictsDescription:
      "请为每个冲突项选择保留完整的本地或远端版本。确认应用前不会修改任何内容。",
    syncConflictsApply: "应用选择",
    syncConflictsPending: "有 {count} 个同步项需要你解决。",
    syncConflictsReview: "查看冲突",
    syncConflictsKeepAllLocal: "全部保留本地",
    syncConflictsUseAllRemote: "全部采用远端",
    syncConflictsSelected: "已选择 {selected} / {total}",
    syncConflictChooseAll: "请为每个冲突选择本地或远端版本。",
    syncConflictEntity: "项目类型",
    syncConflictKeepLocal: "保留本地",
    syncConflictUseRemote: "采用远端",
    syncConflictDeleted: "已删除",
    syncConflictsRefreshRequired: "当前冲突已过期，请刷新同步后重试。",
    username: "用户名",
    password: "密码",
    recordingMode: "录制模式",
    recordingModes: {
      raw: "原始数据 (包含颜色代码)",
      text: "纯文本 (去除颜色代码)",
    },
    confirmations: "确认操作",
    confirmCloseTab: "关闭标签页前确认",
    confirmExitApp: "退出应用前确认",
    debugEnabled: "启用调试日志",
    maxRecentServers: "最近连接显示数量 (欢迎页)",
    tabWidthMode: "标签页宽度模式",
    tabWidthModes: {
      adaptive: "自适应 (按标题)",
      fixed: "固定宽度",
    },
    fixedTabWidth: "固定标签页宽度 (px)",
    fixedTabWidthHint: "可设置范围：{min}-{max} px",
    terminalRightClickMode: "终端右键模式",
    terminalRightClickModes: {
      contextMenu: "右键菜单 (复制/粘贴)",
      selectionCopyPaste: "选中复制 + 右键粘贴",
    },
    terminalCommandBlockBar: "显示命令块彩色竖条",
    terminalRenderer: "终端渲染器",
    terminalRenderers: {
      canvas: "Canvas（默认，稳定）",
      webgl: "WebGL（更快，可能有残留）",
    },
    terminalRendererHint:
      "WebGL 渲染更快，但在全屏 TUI 应用下可能出现画面残留。Canvas 为默认且最稳定。",
    tabNewServerSort: "标签栏加号服务器列表排序",
    tabNewServerSorts: {
      default: "默认排序 (按分组显示)",
      recent: "最近连接",
      connectionCount: "最多连接",
      createdAt: "创建时间",
      updatedAt: "更新时间",
    },
    softwareUpdate: "软件更新",
    updateAutoCheck: "自动检查更新",
    updateAutoCheckHint:
      "开启后，Resh 会在启动后检查 GitHub 稳定版，并每 6 小时检查一次。关于页的手动检查不受此开关影响。",
    updateProxy: "更新代理",
    updateProxyHint: "仅用于更新检查与下载，不影响 SSH 连接。",
    updateProxyMissing:
      "原先选择的代理已被删除，当前将使用直连，请重新选择代理。",
    updateStatusChecking: "正在检查更新…",
    updateStatusUpToDate: "已是最新版本",
    updateStatusAvailable: "发现新版本：{version}",
    updateStatusDownloading: "正在下载更新…",
    updateStatusInstalling: "正在安装更新…",
    updateStatusReady: "更新已下载并校验完成",
    updateStatusWaitingRestart: "正在等待安全重启…",
    updateStatusError: "检查更新失败",
    updateStatusRateLimited: "GitHub API 请求过于频繁，请稍后再试。",
    updatePhaseChecksums: "正在下载校验文件…",
    updatePhaseVerifying: "正在校验安装包…",
    updateCheckNow: "检查更新",
    updateDownload: "下载",
    updateDownloadInstall: "下载并安装",
    updateInstallLater: "关闭",
    updateOpenDialog: "查看更新详情",
    updateCurrentVersion: "当前版本",
    updateTargetVersion: "新版本",
    updateDownloadSize: "下载大小",
    updateRestartConfirmHint:
      "Resh 将断开当前 SSH 连接，等待写操作安全结束后重启并恢复标签页。重启后连接不会保持。",
    updateRestartLaterHint: "保留已校验的安装包并关闭对话框",
    updateRestartNow: "重启并更新",
    updateRestartWaiting: "正在等待安全写操作结束…",
    updateRestartBlockedDirty: "请先保存或放弃未保存的编辑器标签，再重启。",
    updateRestartBlockedSaving: "请等待编辑器保存完成后再重启。",
    updateRestartBlockedRecording: "请先停止终端录制并等待落盘后再重启。",
    updateRestartBlockedSettings: "请先完成或关闭设置（含待保存更改）后再重启。",
    updateRestartCancelWait: "取消重启",
    updateRestartKeepWaiting: "继续等待",
    updateRestartTimeout: "仍有操作在进行。可继续等待或取消重启。",
    updateRestartCategoryConfig: "保存配置",
    updateRestartCategorySync: "WebDAV 同步",
    updateRestartCategoryTransfer: "SFTP 传输",
    updateRestartCategoryEditUpload: "SFTP 编辑上传",
    updateRestorePartial: "已恢复标签，部分失败：{details}",
    updateRestoreSuccess: "更新后已恢复先前会话",
    updateInstallFailed: "更新安装失败：{details}",
    updateInstallSuccess: "已更新到版本 {version}",
    updateTitleButton: "发现新版本",
    updateTitleDownloading: "正在下载更新",
    updateTitleReady: "更新已就绪",
    updateTitleError: "更新出错",
    updateErrorNetwork: "检查更新时网络错误",
    updateErrorIncomplete: "发布缺少必需的安装文件",
    updateErrorProxy: "更新代理无效或不可用",
    saveStatus: {
      saving: "正在保存...",
      saved: "已保存",
      error: "保存出错",
    },
    common: {
      add: "添加",
      edit: "编辑",
      delete: "删除",
      save: "保存",
      cancel: "取消",
      name: "名称",
      path: "路径",
      host: "主机",
      port: "端口",
      type: "类型",
      none: "无",
      proxy: "代理",
      noProxy: "无代理 (直连)",
      remove: "移除",
      rename: "重命名",
      create: "创建",
      apply: "应用",
      loading: "正在加载配置...",
      connect: "连接",
      actions: "操作",
      optional: "可选",
      syncThisItem: "同步此项",
      enableSync: "开启同步",
      queued: "等待中",
    },
    serverTab: {
      title: "SSH 服务器",
      addServer: "添加服务器",
      editServer: "编辑服务器",
      emptyState: "暂无服务器。点击添加开始使用。",
      defaultGroup: "默认分组",
      connectTooltip: "连接服务器",
      editTooltip: "编辑服务器",
      deleteTooltip: "删除服务器",
      deleteConfirmation: "确定要删除此服务器吗？",
      deleteInUseConfirmation:
        "此服务器正被一个或多个服务器用作跳板机。确定要删除吗？受影响的服务器的跳板机设置将被清空。",
      jumphost: "跳板机",
    },
    authTab: {
      title: "身份验证管理",
      addAuth: "添加身份验证",
      editAuth: "编辑身份验证",
      emptyState: "暂无身份验证。点击添加开始使用。",
      editTooltip: "编辑身份验证",
      deleteTooltip: "删除身份验证",
      deleteInUseConfirmation:
        "此身份验证正被一个或多个服务器使用。确定要删除吗？受影响的服务器的身份验证将被设置为“无”。",
      passwordType: "密码",
      keyType: "SSH 密钥",
    },
    proxyTab: {
      title: "代理服务器",
      addProxy: "添加代理",
      editProxy: "编辑代理",
      emptyState: "暂无代理。点击添加开始使用。",
      editTooltip: "编辑代理",
      deleteTooltip: "删除代理",
      deleteInUseConfirmation:
        "此代理正被一个或多个服务器使用。确定要删除吗？受影响的服务器的代理设置将被清空。",
    },
    snippetsTab: {
      title: "Snippets",
      addSnippet: "添加代码段",
      editSnippet: "编辑代码段",
      emptyState: "暂无代码段。点击添加开始使用。",
      editTooltip: "编辑代码段",
      deleteTooltip: "删除代码段",
      deleteConfirmation: "确定要删除此代码段吗？",
    },
    serverForm: {
      nameLabel: "服务器名称",
      namePlaceholder: "例如：生产服务器、AWS 实例",
      groupLabel: "分组",
      groupPlaceholder: "例如：生产环境、测试环境",
      hostPlaceholder: "例如：example.com 或 192.168.1.100",
      usernameLabel: "用户名",
      usernamePlaceholder: "例如：ubuntu",
      authLabel: "身份验证",
      authPlaceholder: "请选择身份验证...",
      routingTitle: "连接路由 (可选)",
      proxyLabel: "代理服务器",
      jumphostLabel: "跳板机 (SSH 隧道)",
      portForwardingTitle: "端口转发",
      localPortPlaceholder: "本地端口",
      remotePortPlaceholder: "远程端口",
      keepaliveTitle: "保持激活 (Keepalive)",
      keepaliveInterval: "发送间隔 (秒，0 为禁用)",
      autoExecTitle: "自动执行命令",
      addCommand: "添加命令",
      autoExecPlaceholder: "例如：cd /var/log",
      favoritesTab: "收藏",
      sftpFavoritePathsTitle: "SFTP 收藏路径",
      sftpFavoritePathsHint:
        "这些路径会保存到当前服务器，并用于 SFTP 侧边栏收藏菜单。",
      sftpFavoritePathPlaceholder: "例如：/var/log/nginx",
      sftpFavoritePathsEmpty: "当前服务器还没有收藏路径。",
      addFavoritePath: "添加",
      envVarsTitle: "环境变量",
      varNamePlaceholder: "变量名",
      varValuePlaceholder: "值",
    },
    authForm: {
      nameLabel: "名称",
      namePlaceholder: "例如：我的开发机、公司服务器密码",
      typeLabel: "验证类型",
      passwordLabel: "密码",
      passwordPlaceholder: "请输入密码",
      keyContentLabel: "SSH 密钥内容",
      uploadKey: "上传密钥文件",
      keyContentPlaceholder: "请粘贴 SSH 私钥 (-----BEGIN PRIVATE KEY-----...)",
      passphraseLabel: "密钥密码 (可选)",
      passphrasePlaceholder: "如果密钥已加密，请输入密码",
    },
    proxyForm: {
      nameLabel: "代理名称",
      namePlaceholder: "例如：公司代理、家庭 SOCKS5",
      typeLabel: "代理类型",
      hostLabel: "主机地址",
      hostPlaceholder: "例如：proxy.example.com",
      portLabel: "端口",
      authTitle: "身份验证 (可选)",
      usernamePlaceholder: "无需验证请留空",
      passwordPlaceholder: "无需验证请留空",
      ignoreSslLabel: "忽略 SSL 证书错误",
      ignoreSslDesc: "用于公司代理 MITM 拦截场景",
    },
    snippetForm: {
      nameLabel: "名称",
      namePlaceholder: "代码段名称",
      contentLabel: "内容 (Bash/命令)",
      contentPlaceholder: "#!/bin/bash...",
      descriptionLabel: "描述 (可选)",
      descriptionPlaceholder: "简要描述此代码段的功能",
      groupLabel: "分组",
      groupPlaceholder: "分组名称",
      defaultGroup: "默认",
    },
    manualAuth: {
      title: "手动身份验证",
      retryTitle: "认证失败",
      retryDescription: "{server} 认证失败，请重试：",
      enterCredentials: "请输入 {server} 的凭据：",
      usernameLabel: "用户名",
      passwordLabel: "密码",
      passwordPlaceholder: "如使用密钥则留空",
      orDivider: "— 或 —",
      privateKeyLabel: "私钥 (PEM)",
      privateKeyPlaceholder: "在此粘贴您的私钥...",
      rememberCredentials: "记住认证信息",
    },
    welcome: {
      title: "欢迎使用 Resh",
      subtitle: "一款专业、高性能的 SSH 客户端。",
      recentTitle: "最近连接",
      viewAll: "查看全部",
      newConnection: "新建连接",
      configureNew: "配置新的服务器",
      noServers: "暂无服务器",
      getFirstStarted: "添加您的第一个服务器，开始体验 Resh。",
    },
    mainWindow: {
      closeTab: "关闭标签页",
      cloneTab: "克隆标签页",
      closeOthers: "关闭其他标签页",
      reconnect: "重新连接",
      settings: "设置",
      snippets: "代码段",
      aiAssistant: "AI 助手",
      snippetsTooltip: "代码段",
      exportLogs: "导出会话",
      startRecording: "开始录制",
      stopRecording: "停止录制",
      tabAriaLabel: "第 {index} 个标签页，共 {total} 个",
      splitView: "分屏",
      exitSplitView: "退出分屏",
      splitLeftRight: "左右分屏",
      splitTopBottom: "上下分屏",
      splitFourGrid: "四宫格分屏",
      splitSelectTabsTitle: "选择要加入分屏的标签",
      splitSelectTabsHint: "请为 {layout} 选择 {count} 个标签",
      splitSelectedCount: "已选择 {selected}/{required}",
      editorUnsavedTitle: "未保存的更改",
      editorUnsavedSwitch: "“{name}”有未保存更改，切换标签前是否保存？",
      editorUnsavedClose: "“{name}”有未保存更改，关闭标签前是否保存？",
      discardChanges: "放弃更改",
      editor: {
        invalidPayload: "编辑器载荷无效。",
        documentMissing: "未找到编辑器文档。",
        saveFailed: "保存编辑器文件失败: {error}",
      },
      editorConflict: {
        title: "远程文件已变化",
        description:
          "编辑 {path} 期间远程文件已变化。你的本地修改已保留，且尚未保存到远程。",
        remoteChanged: "远程文件元数据已变化。",
        remoteDeleted: "远程文件已被删除。",
        remotePath: "远程路径",
        remoteSize: "远程大小",
        remoteModified: "远程修改时间",
        notAvailable: "不可用",
        snapshotUnavailable: "无法读取当前远程文本: {error}",
        snapshotUnavailableShort: "当前远程文本不可用。",
        localVersion: "你的本地修改",
        remoteVersion: "当前远程版本",
        viewDiff: "查看差异",
        hideDiff: "隐藏差异",
        adoptRemote: "采用远程版本",
        overwriteRemote: "覆盖远程文件",
        overwriteConfirm:
          "这会用你的本地修改替换当前远程文件，且无法撤销。",
        overwriteConfirmAction: "立即覆盖远程文件",
        adopted: "已用当前远程版本替换编辑器内容。",
        conflictStillCurrent: "远程文件再次变化，请先查看最新版本再覆盖。",
      },
      externalEditConflict: {
        title: "外部编辑自动上传已暂停",
        description:
          "本地编辑 {path} 时远程文件已变化，自动上传已暂停。",
        autoUploadPaused: "该文件的自动上传已暂停。",
        remoteChanged: "远程文件元数据已变化。",
        remoteDeleted: "远程文件已被删除。",
        pendingLocal: "你的最新本地保存已合并为待上传版本。",
        remotePath: "远程路径",
        localPath: "本地临时文件",
        remoteSize: "远程大小",
        remoteModified: "远程修改时间",
        notAvailable: "不可用",
        snapshotUnavailable: "无法完整加载当前远程 revision: {error}",
        changedHint:
          "可采用远程版本覆盖本地临时文件，或用本地文件覆盖远程。保持暂停会维持自动上传关闭，直到你决定。",
        deletedHint:
          "远程文件已不存在。可保持暂停，或明确选择从本地临时文件重新创建远程文件。",
        keepPaused: "保持暂停",
        adoptRemote: "采用远程版本",
        overwriteRemote: "覆盖远程文件",
        recreateRemote: "重新创建远程文件",
        overwriteConfirm:
          "这会用本地临时文件替换当前远程文件，且无法撤销。",
        overwriteConfirmAction: "立即覆盖远程文件",
        recreateConfirm: "这会根据本地临时文件重新创建远程文件。",
        recreateConfirmAction: "立即重新创建",
        adopted: "已用远程版本替换本地临时文件，并恢复自动上传。",
        overwrote: "已上传本地临时文件，并恢复自动上传。",
        keptPaused: "该文件的自动上传仍保持暂停。",
        conflictStillCurrent: "远程文件再次变化，请先查看最新冲突再覆盖。",
        resolveFailed: "处理外部编辑冲突失败: {error}",
        bannerTitle: "外部编辑上传已暂停",
        bannerAction: "处理",
      },
    },
    windowControls: {
      minimize: "最小化",
      maximize: "最大化",
      close: "关闭",
    },
    terminalTab: {
      connecting: "正在连接服务器 {name}...",
      connected: "连接成功！会话 ID: {id}",
      connectionClosed: "连接已断开。",
      error: "错误: {error}",
      hostCopied: "主机地址已复制到剪贴板",
      contextMenuCopied: "已复制到剪贴板",
      contextMenuCopy: "复制",
      contextMenuPaste: "粘贴",
    },
    editorTab: {
      undo: "撤销",
      redo: "重做",
      save: "保存",
      wordWrap: "换行",
      wordWrapEnable: "开启自动换行",
      wordWrapDisable: "关闭自动换行",
      language: "文档类型",
      encoding: "编码: {encoding}",
      dirty: "已修改",
      saved: "已保存",
      remoteChanged: "远程已变化",
      lineColumn: "行 {line}, 列 {column}",
    },
    newTabButton: {
      connectTo: "连接到服务器",
      quickConnectPlaceholder: "快速连接：ip、host 或 user@host",
      quickConnectAction: "快速连接",
      quickConnectInvalid: "请输入 ip、host 或 user@host",
      manageServers: "服务器管理",
      collapseGroup: "收起分组",
      expandGroup: "展开分组",
    },
    ai: {
      sidebarTitle: "AI 助手",
      tabTitle: "AI",
      configuration: "配置",
      newChat: "新对话",
      history: "历史记录",
      clearHistory: "清空历史",
      typeMessage: "输入消息...",
      thinkingProcess: "思考过程",
      stopGeneration: "停止生成",
      startChat: "开始对话",
      copyMessage: "复制消息",
      regenerateMessage: "重新生成回复",
      requestFailed: "AI 请求失败: {error}",
      unknownError: "未知错误",
      noHistory: "暂无历史记录",
      deleteSessionConfirm: "确定要删除此会话吗？",
      clearHistoryConfirm: "确定要清空此服务器的所有历史记录吗？",
      selectServer: "请选择一个服务器并开始新对话。",
      editorContext: {
        includeCurrentFile: "附带当前编辑文件",
        fileLabel: "文件: {path}",
        languageLabel: "语言: {language}",
        charCountLabel: "字符: {count}",
        readOnlyHint: "将作为只读上下文发送，仅用于分析。",
        tooLarge:
          "当前编辑内容为 {count} 个字符，超过上限 {max}。请关闭此选项或减少文件内容后再发送。",
      },
      globalAdditionalPrompt: "全局附加提示词",
      globalAdditionalPromptDesc:
        "此提示词将附加到所有 AI 请求的系统提示词中。支持跨设备同步。",
      globalAdditionalPromptPlaceholder: "例如：始终使用中文回复。保持简洁。",
      serverAdditionalPrompt: "服务器附加提示词",
      serverAdditionalPromptDesc:
        "此提示词专用于当前服务器，将附加在全局提示词之后。",
      serverAdditionalPromptPlaceholder:
        "例如：此服务器运行 RHEL 8 且安装了 Podman。优先使用 podman 命令。",
      maxChatContext: "最大对话上下文",
      requestTimeout: "请求超时时间 (秒)",
      toolConfirmationCountdown: "工具确认倒计时 (秒)",
      toolConfirmationCountdownYoloWarning:
        "0 = YOLO 模式：所有工具调用（含危险工具）将直接执行，无需确认。高风险命令（rm、dd、mkfs、fdisk、reboot、shutdown、halt、poweroff、init）仍需确认。",
      yoloModeBadge: "YOLO 模式已开启 - 不进行工具确认",
      channels: "AI 渠道",
      models: "AI 模型",
      addChannel: "添加渠道",
      addModel: "添加模型",
      noChannels: "暂无 AI 渠道配置。",
      noModels: "暂无 AI 模型配置。",
      editChannel: "编辑 AI 渠道",
      editModel: "编辑 AI 模型",
      deleteChannelConfirm: "确定要删除此 AI 渠道吗？关联的模型也会受到影响。",
      deleteModelConfirm: "确定要删除此 AI 模型吗？",
      disabled: "已禁用",
      channelDisabled: "渠道已禁用",
      channelForm: {
        name: "名称",
        namePlaceholder: "例如：OpenAI 个人",
        type: "类型",
        endpoint: "接口地址 (可选)",
        apiKey: "API Key (可选)",
        active: "启用此渠道",
      },
      modelForm: {
        name: "模型名称",
        namePlaceholder: "例如：gpt-4, gpt-3.5-turbo",
        channel: "所属渠道",
        active: "启用此模型",
        contextWindow: "最大上下文 (tokens)",
        responseReserve: "响应预留 (tokens)",
      },
      modelCatalog: "模型目录 (models.dev)",
      catalogModels: "个模型",
      catalogProviders: "个 Provider",
      catalogFresh: "新鲜",
      catalogStale: "已过期",
      catalogFetched: "上次更新",
      catalogNextRefresh: "下次自动更新",
      catalogLoading: "目录状态加载中…",
      catalogRefresh: "立即更新目录",
      catalogRefreshing: "更新中…",
      catalogRefreshOk: "目录更新成功",
      catalogRefreshFailed: "目录更新失败",
      catalogLookupBusy: "正在查询 models.dev 目录…",
      catalogLookupError: "目录查询失败",
      catalogLookupMatch: "已从 models.dev 目录获取",
      catalogLookupContext: "最大上下文",
      catalogLookupOutput: "输出预留",
      catalogLookupNone: "models.dev 目录中未找到该模型，可手动填写",
      catalogLookupDuplicates: "该 ID 在 {count} 个 Provider 下存在，已优先采用：",
      tool: {
        confirmExecution: "确认执行",
        autoExecute: "{seconds}秒后自动执行",
        cancel: "取消",
        decline: "拒绝",
        allowOnce: "仅本次允许",
        allowForSession: "本会话内允许",
        cancelRun: "取消当前运行",
        resolving: "正在等待后端确认…",
        runNow: "立即运行 ({seconds}s)",
        confirmRun: "确认运行",
        usingTools: "使用工具: {tools}",
        codeCopied: "代码已复制",
        copyCode: "复制代码",
        insertToTerminal: "插入终端",
        executeCommand: "执行命令",
        executeBackgroundCommand: "后台执行命令",
        getTerminalOutput: "读取终端输出",
        getSelectedTerminalOutput: "读取终端选中文本",
        readFile: "读取远程文件",
        sendInterrupt: "发送中断 (Ctrl+C)",
        sendTerminalInput: "发送终端输入",
        sftpDownload: "SFTP 下载",
        sftpUpload: "SFTP 上传",
        timeoutSeconds: "超时时间: {seconds}秒",
        waitFinish: "等待命令结束: {value}",
        waitFinishOn: "是",
        waitFinishOff: "否",
        commandOutput: "命令输出",
        noCommandOutput: "未捕获到输出",
      },
    },
    sftp: {
      title: "SFTP",
      transfers: "传输任务",
      noFiles: "暂无文件",
      notConnected: "未连接",
      loading: "加载中...",
      sort: {
        name: "名称",
        dateModified: "修改日期",
      },
      contextMenu: {
        open: "打开",
        download: "下载",
        upload: "上传",
        newFile: "新建文件",
        newFolder: "新建文件夹",
        properties: "属性",
        rename: "重命名",
        delete: "删除",
        path: "路径",
        copyFileName: "复制文件名",
        copyFolderName: "复制文件夹名",
        copyFullPath: "复制完整路径",
        sendPathToTerminal: "发送路径到终端",
        terminalJump: "终端跳转到此路径",
        edit: "编辑",
        editServerVim: "使用服务器 Vim 打开",
        editLocal: "使用本地编辑器打开",
        copy: "复制",
        cut: "剪切",
        pasteCopy: "粘贴副本",
        pasteMove: "移动到此处",
        cancel: "取消",
        commands: "命令",
        addFavorite: "收藏此目录",
      },
      tooltips: {
        sort: "排序",
        favorites: "收藏目录",
        refresh: "刷新",
        lock: "锁定侧边栏",
        unlock: "解锁侧边栏",
        close: "关闭",
      },
      favorites: {
        empty: "暂无收藏目录",
        remove: "删除收藏路径",
        pathNotFound: "收藏路径不存在：{path}",
      },
      editor: {
        serverUnavailable: "此文件对应的服务器不可用。",
        nonTextFile: "该文件不是文本文件，无法在编辑器中打开。",
        openFailed: "在编辑器中打开文件失败: {error}",
        unknownError: "未知错误",
      },
      modals: {
        deleteConfirmTitle: "删除确认",
        deleteConfirmMessage: "确定要删除此项吗？此操作无法撤销。",
        copyFallbackTitle: "使用终端复制？",
        copyFallbackMessage:
          "服务器不支持服务端复制。是否在关联终端中使用 cp 命令复制？如果不同意，将回退到客户端流式复制。",
        copyFallbackUseCp: "使用 cp 命令",
        copyFallbackUseStreaming: "使用流式复制",
        newFileTitle: "新建文件",
        newFolderTitle: "新建文件夹",
        renameTitle: "重命名",
        propertiesTitle: "属性",
        permissionsLabel: "权限 (八进制)",
        permissionsPlaceholder: "例如：755",
        itemNameLabel: "名称",
      },
      settings: {
        defaultDownloadPath: "默认下载路径",
        defaultDownloadPathPlaceholder: "例如：C:\\Users\\User\\Downloads",
        defaultDownloadPathDesc: "SFTP 文件下载的默认存放目录。",
        maxConcurrentTransfers: "最大并行传输任务数",
        maxConcurrentTransfersDesc:
          "同时进行的下载/上传任务数量。推荐值：2-4。",
        maxConcurrentTransfersPerSession: "单会话最大并行任务数",
        maxConcurrentTransfersPerSessionDesc:
          "单个 SSH 会话允许的并行传输任务上限，用于保证队列公平性。",
        transferProfile: "传输策略",
        transferProfileDesc:
          "推荐使用 balanced。safe 在不稳定链路下更稳，fast 优先追求吞吐。",
        profileSafe: "Safe（保守）",
        profileBalanced: "Balanced（推荐）",
        profileFast: "Fast（激进）",
        downloadMaxInflight: "下载最大并发请求",
        downloadMaxInflightDesc: "下载流水线阶段使用的读取并发上限。",
        uploadMaxInflight: "上传最大并发请求",
        uploadMaxInflightDesc: "单任务上传时并发写入块请求上限。",
        chunkSizeMinKb: "分块最小值 (KB)",
        chunkSizeMaxKb: "分块最大值 (KB)",
        chunkSizeDesc: "传输调参使用的分块范围，最终值会受服务端能力限制。",
        enableMultiConnectionForSmallFiles: "启用小文件多连接模式",
        enableLargeFileStriping: "启用大文件分段模式（实验）",
        experimentalTuningDesc: "实验开关默认关闭，后续版本可能继续调整策略。",
        browse: "选择文件夹",
        editorAssociations: "编辑器关联",
        addRule: "添加规则",
        filePattern: "文件名模式",
        editorPath: "编辑器路径",
        browseFile: "选择编辑器",
        noRules: "暂无关联规则。添加规则以将文件类型关联到本地编辑器。",
        patternPlaceholder: "*.log",
        editorPlaceholder: "C:\\Windows\\notepad.exe",
        priorityHint: "提示：拖拽规则可调整优先级。靠前的规则优先级更高。",
        customCommands: "自定义命令",
        addCommand: "添加命令",
        commandSync: "同步",
        commandName: "名称",
        commandPattern: "匹配模式",
        commandExec: "执行命令",
        commandPlaceholderName: "名称",
        commandPlaceholderPattern: "*.log",
        commandPlaceholderExec: "tail -f {fpath}",
        commandHelp:
          "使用 {fpath} 代表文件绝对路径，{dpath} 代表上级目录路径，{fname} 代表无后缀的文件名。",
        noCommands: "暂无自定义命令",
        synced: "已同步",
        localOnly: "仅本地",
      },
    },
    about: {
      tabTitle: "关于",
      description:
        "一款基于 Tauri 2 + React 19 构建的现代、安全的多标签 SSH 客户端。",
      version: "版本",
      author: "作者",
      github: "GitHub 仓库",
      license: "许可证",
      techStack: "技术栈",
      thanks: "特别感谢所有贡献者和开源社区。",
    },
  },
}

export type LocaleType = keyof typeof locales
