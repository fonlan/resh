...
export const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${className}`} id={id}>
      <div
        className={`w-full px-3 py-2 bg-[var(--bg-primary)] border border-[var(--glass-border)] rounded-md text-[var(--text-primary)] text-[13px] font-[var(--font-ui)]  leading-5 transition-all text-left flex justify-between items-center cursor-pointer outline-none focus:border-[var(--accent-primary)] focus:shadow-[0_0_0_2px_rgba(59,130,246,0.1)] ${disabled ? "opacity-50 cursor-not-allowed pointer-events-none" : ""} ${triggerClassName}`}
      ...
      >
        <span
          className={`whitespace-nowrap overflow-hidden text-ellipsis flex-1 min-w-0 mr-2 ${!selectedOption ? "text-zinc-400" : ""}`}
        >
          {selectedOption ? (
            <EmojiText text={selectedOption.label} />
          ) : (
            placeholder
          )}
        </span>
