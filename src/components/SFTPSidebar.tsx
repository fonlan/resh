...
export const SFTPSidebar: React.FC<SFTPSidebarProps> = ({
  ...
  const [copyFallbackModal, setCopyFallbackModal] =
    useState<CopyFallbackModalState>({
      isOpen: false,
      sessionId: "",
      sourcePath: "",
      destPath: "",
  ...
  const [propertiesModal, setPropertiesModal] = useState<{
    ...
    entry: FileEntry | null
    permissions: string
  }>({ isOpen: false, entry: null, permissions: "" })
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (
        showSortMenu &&
        !(e.target as Element).closest(".sftp-sort-menu") &&
        !(e.target as Element).closest(".sftp-sort-btn")
      ) {
  ...
  const refreshDirectoryTree = useCallback(
    async (
      ...
      const childPathsToLoad = sortedPaths.filter(
        (expandedPath) =>
          expandedPath !== "/" &&
          expandedPath !== "." &&
      ...
      try {
        const batchResults = await invoke<DirectoryListResult[]>(
          "sftp_list_dirs_sorted",
          {
            sessionId: sid,
  ...
  const handleRefresh = async () => {
    ...
    const expandedPaths = currentSession
      ? getAllExpandedPaths(currentSession.rootFiles)
      : new Set<string>()
  ...
  useEffect(() => {
    if (isOpen && sessionId) {
      ...
      const shouldLoad =
        !sessionState ||
        (sessionState.rootFiles.length === 0 && !sessionState.isLoading)
  ...
  const handleSort = (type: SortType) => {
    ...
    const newOrder: SortOrder =
      session.sortState.type === type && session.sortState.order === "asc"
        ? "desc"
        : "asc"
  ...
  const handleTreeItemDragStart = useCallback(
    (e: React.DragEvent<HTMLButtonElement>, entry: FileEntry) => {
      ...
      e.dataTransfer.setData(
        ...
        JSON.stringify({
          ...
          isDir:
            entry.is_dir ||
            Boolean(entry.is_symlink && entry.target_is_dir === true),
        }),
      )
  ...
  const handleExecuteCustomCommand = (cmd: SftpCustomCommand) => {
    ...
    window.dispatchEvent(
      new CustomEvent("paste-snippet", { detail: command + "\r" }),
    )
    handleCloseContextMenu()
  }
  ...
  const handleEditLocal = async () => {
    ...
    try {
      ...
      if (config.general.sftp?.editors) {
        const rule = config.general.sftp.editors.find((r) =>
          matchPattern(entryName, r.pattern),
        )
        if (rule) {
      ...
      console.log("[handleEditLocal] Invoking open_local_editor", {
        path: localPath,
        editor: editorCmd,
      })
  ...
  const filterExpandedPathsBySubtree = (
    expandedPaths: Set<string>,
    basePath: string,
  ): Set<string> => {
  ...
  const refreshRenamedTreeNode = async (oldPath: string, newPath: string) => {
    ...
    const expandedPaths = getAllExpandedPaths(currentSessionState.rootFiles)
    const remappedExpandedPaths = remapExpandedPathsAfterRename(
      expandedPaths,
      oldPath,
      newPath,
    )
    const subtreeExpandedPaths = filterExpandedPathsBySubtree(
      remappedExpandedPaths,
      normalizedParentPath,
    )
  ...
  const confirmDelete = async () => {
    ...
    try {
      await invoke("sftp_delete", {
        sessionId,
        path: deleteModal.entry.path,
        isDir: deleteModal.entry.is_dir,
      })
  ...
  const handleNewFile = () => {
    if (!contextMenu || !contextMenu.entry) return
    const parentPath = isDirectory(contextMenu.entry)
      ? contextMenu.entry.path
  ...
  const confirmProperties = async () => {
    ...
    try {
      ...
      await invoke("sftp_chmod", {
        sessionId,
        path: propertiesModal.entry.path,
        mode: permNum,
      })
  ...
  const handleSendPath = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("paste-snippet", {
        detail: quotePathForTerminalInput(contextMenu.entry.path),
      }),
    )
    handleCloseContextMenu()
  }
  ...
  const handlePaste = async () => {
    ...
    const sourceParentPath =
      clipboard.sourcePath.substring(
        0,
        clipboard.sourcePath.lastIndexOf("/"),
      ) || "/"
  ...
  const handleResolveConflict = async (
    ...
    resolution: ConflictResolution | "overwrite-all",
  ) => {
    try {
