...
export const SplitTabPickerModal: React.FC<SplitTabPickerModalProps> = ({
  ...
  useEffect(() => {
    ...
    const nextSelected = initialSelectedTabIds.filter(id => existingIds.has(id)).slice(0, requiredCount)
  ...
  const layoutLabel = useMemo(() => {
    ...
    return ''
  }, [layout, t.mainWindow.splitLeftRight, t.mainWindow.splitTopBottom, t.mainWindow.splitFourGrid])
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1200] bg-black/55 backdrop-blur-[1px] flex items-center justify-center p-4" onMouseDown={onCancel}>
      <div
      ...
      >
        <div className="px-5 py-4 border-b border-[var(--glass-border)] bg-[rgba(255,255,255,0.02)]">
          <h3 className="m-0 text-[16px] font-semibold text-[var(--text-primary)]">{t.mainWindow.splitSelectTabsTitle}</h3>
        ...
        <div className="p-4 max-h-[380px] overflow-y-auto flex flex-col gap-2">
          {tabs.map(tab => {
            ...
            const disableUnchecked = !selected && selectedTabIds.length >= requiredCount
            ...
            return (
              ...
              >
                <span className={`w-[18px] h-[18px] rounded-[5px] border flex items-center justify-center ${selected ? 'border-[var(--accent-primary)] bg-[var(--accent-primary)] text-white' : 'border-[var(--glass-border)] bg-[var(--bg-secondary)] text-transparent'}`}>
                  <Check size={12} />
                </span>
