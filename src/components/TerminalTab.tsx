...
export const TerminalTab = React.memo<TerminalTabProps>(
  ({
    ...
    const [terminalContextMenu, setTerminalContextMenu] =
    ...
    const inputFlushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null,
    )
    ...
    const flushQueuedInput = useCallback(async () => {
      if (
        isFlushingQueuedInputRef.current ||
    ...
    const handleData = useCallback(
      (data: string) => {
        const { newBuffer, commandExecuted, shouldFlushImmediately } =
          processInputBuffer(data, inputBufferRef.current)
    ...
    const handleResize = useCallback((cols: number, rows: number) => {
      if (sessionIdRef.current) {
        invoke("resize_terminal", {
          params: { session_id: sessionIdRef.current, cols, rows },
        }).catch(() => {
    ...
    const [manualCredentials, setManualCredentials] =
      useState<ManualAuthCredentials>({
    ...
    useEffect(() => {
      ...
      const connect = async (manualCreds?: ManualAuthCredentials) => {
        try {
          ...
          const connectingMsg = currentT.terminalTab.connecting.replace(
            "{name}",
            currentServer.name,
          )
          ...
          if (
            !manualCreds &&
            (!currentServer.username || !currentServer.authId)
          ) {
          ...
          if (!manualCreds) {
            const auth = authenticationsRef.current.find(
              (a) => a.id === currentServer.authId,
            )
          ...
          if (currentServer.jumphostId) {
            const jhServer = serversRef.current.find(
              (s) => s.id === currentServer.jumphostId,
            )
            if (jhServer) {
          ...
          if (!proxy) {
            proxy = proxiesRef.current.find(
              (p) => p.id === currentServer.proxyId,
            )
          }
          ...
          if (currentServer.jumphostId) {
            ...
            if (jhServer) {
              const jhAuth = authenticationsRef.current.find(
                (a) => a.id === jhServer.authId,
              )
          ...
          setIsConnected(true)
          const connectedMsg = currentT.terminalTab.connected.replace(
            "{id}",
            sid,
          )
          ...
          outputUnlistener = await listen<string>(
            `terminal-output:${sid}`,
            (event) => {
          ...
          const errorStr = String(err)
          const errorMsg = tRef.current.terminalTab.error.replace(
            "{error}",
            errorStr,
          )
          writeRef.current("\r\n" + errorMsg + "\r\n")
      ...
      if (connectTrigger > 0) {
        ...
        connect(
          currentManualCredentials.password ||
            currentManualCredentials.privateKey
    ...
    const handleTerminalDrop = useCallback(
      (e: React.DragEvent<HTMLDivElement>) => {
        ...
        window.dispatchEvent(
          new CustomEvent("paste-snippet", {
            detail: quotePathForTerminalInput(sftpPath),
          }),
        )
        focus()
      },
    ...
    useEffect(() => {
      ...
      window.addEventListener(
        `export-terminal-logs:${tabId}`,
        handleExportLogs as EventListener,
      )
      return () => {
        window.removeEventListener(
          `export-terminal-logs:${tabId}`,
    ...
    useEffect(() => {
      ...
      window.addEventListener(
        `start-recording:${tabId}`,
        handleStartRecording as unknown as EventListener,
      )
      window.addEventListener(
        `stop-recording:${tabId}`,
        handleStopRecording as unknown as EventListener,
      )
    ...
    useEffect(() => {
      const handleReconnect = async () => {
        ...
        try {
          ...
          setIsConnected(true)
          const reconnectedMsg = t.terminalTab.connected.replace(
            "{id}",
            sessionIdRef.current,
          )
      ...
      window.addEventListener(
        `reconnect:${tabId}`,
        handleReconnect as unknown as EventListener,
      )
    ...
    return (
      ...
      >
        <StatusBar
          leftText={statusText}
          rightText={
            server.username ? `${server.username}@${server.host}` : server.host
          }
          rightCopyText={server.host}
          onRightCopySuccess={() =>
            onShowToast?.(t.terminalTab.hostCopied, "success")
          }
          theme={theme}
