...
export const AITab: React.FC<AITabProps> = ({
  ...
  // Copilot Auth State
  const [copilotAuthData, setCopilotAuthData] =
  ...
  const pollCopilotAuth = async (data: DeviceCodeResponse) => {
    ...
    const pollLoop = async () => {
      ...
      try {
        const token = await invoke<string>("poll_copilot_auth", {
  ...
  const handleSaveChannel = () => {
    ...
    if (editingChannel) {
      const updatedChannel: AIChannel = {
        ...
        isActive,
        synced:
          channelFormData.synced !== undefined
            ? channelFormData.synced
            : editingChannel.synced,
        updatedAt: now,
      }
      onAIChannelsUpdate(
        aiChannels.map((c) =>
          c.id === editingChannel.id ? updatedChannel : c,
        ),
      )
    } else {
      const newChannel: AIChannel = {
        ...
        isActive,
        synced:
          channelFormData.synced !== undefined ? channelFormData.synced : true,
        updatedAt: now,
      }
  ...
  const handleSaveModel = () => {
    ...
    if (editingModel) {
      ...
      onAIModelsUpdate(
        aiModels.map((m) => (m.id === editingModel.id ? updatedModel : m)),
      )
    } else {
    ...
    <div className="w-full max-w-full">
      ...
      <div className="mb-8">
        <div className="flex flex-col gap-1.5 mb-4">
          <label
            htmlFor="ai-additional-prompt"
            className="block text-sm font-medium text-zinc-400 mb-1.5 "
          >
            {t.ai.globalAdditionalPrompt}
          </label>
      ...
      <div className="flex flex-col gap-2 mb-8">
        {sortedChannels.length === 0 ? (
          ...
          sortedChannels.map((channel) => (
            <div
              key={channel.id}
              className={`flex items-center justify-between p-3 bg-[var(--bg-primary)] border-[1.5px] border-zinc-700/50 rounded-md transition-all gap-3 ${!channel.isActive ? "opacity-60" : ""} hover:border-blue-500 hover:shadow-[0_0_20px_rgba(59,130,246,0.2)] hover:-translate-y-px`}
            >
              <div className="flex flex-col gap-1 flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-[var(--text-primary)] m-0 whitespace-nowrap overflow-hidden text-overflow-ellipsis">
                    {channel.name}
                  </span>
                  {!channel.isActive && (
                    <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-[var(--bg-primary)] text-[var(--text-muted)] rounded border border-zinc-700/50 whitespace-nowrap">
                      {t.ai.disabled}
                    </span>
                  )}
                </div>
                <p className="text-xs text-[var(--text-muted)] m-0 whitespace-nowrap overflow-hidden text-overflow-ellipsis">
                  Type: {channel.type}
                </p>
              </div>
        ...
        {sortedModels.length === 0 ? (
          ...
          sortedModels.map((model) => {
            ...
            return (
              <div
                key={model.id}
                className={`flex items-center justify-between p-3 bg-[var(--bg-primary)] border-[1.5px] border-zinc-700/50 rounded-md transition-all gap-3 ${!effectivelyEnabled ? "opacity-60" : ""} hover:border-blue-500 hover:shadow-[0_0_20px_rgba(59,130,246,0.2)] hover:-translate-y-px`}
              >
                <div className="flex flex-col gap-1 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-[var(--text-primary)] m-0 whitespace-nowrap overflow-hidden text-overflow-ellipsis">
                    ...
                    {model.enabled && !isChannelActive && (
                      <span
                        className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-yellow-900/50 text-yellow-200 rounded border border-zinc-700/50 whitespace-nowrap"
                        title="Parent channel is disabled"
                      >
                  ...
                  <p className="text-xs text-[var(--text-muted)] m-0 whitespace-nowrap overflow-hidden text-overflow-ellipsis">
                    Channel: {channel ? channel.name : "Unknown Channel"}
                  </p>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          <input
            ...
            disabled={
              channelFormData.type === "copilot" && !channelFormData.apiKey
            }
          />
          <label htmlFor="channel-active" className="text-sm cursor-pointer">
            {t.ai.channelForm.active}
          </label>
        </div>
        ...
        {channelFormData.type === "copilot" ? (
            ...
            {channelFormData.apiKey ? (
              <div className="flex items-center gap-2 text-green-400 mb-4">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setChannelFormData({ ...channelFormData, apiKey: "" })
                  }
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="text-sm text-zinc-300">
                      ...
                      <button
                        ...
                        onClick={() =>
                          invoke("open_url", {
                            url: copilotAuthData.verification_uri,
                          })
                        }
                      ...
                      >
                        {copilotAuthData.verification_uri}{" "}
          ...
          <>
            <div className="flex flex-col gap-1.5 mb-4">
              <label
                htmlFor="channel-endpoint"
                className="block text-sm font-medium text-zinc-400 mb-1.5 "
              >
                {t.ai.channelForm.endpoint}
              </label>
              <input
                id="channel-endpoint"
                type="text"
            ...
            <div className="flex flex-col gap-1.5 mb-4">
              <label
                htmlFor="channel-apikey"
                className="block text-sm font-medium text-zinc-400 mb-1.5 "
              >
                {t.ai.channelForm.apiKey}
              </label>
              <div className="relative">
                ...
                <button
                  ...
                  aria-label={
                    isChannelApiKeyVisible ? "Hide API key" : "Show API key"
                  }
                  title={
                    isChannelApiKeyVisible ? "Hide API key" : "Show API key"
                  }
                >
                  {isChannelApiKeyVisible ? (
                    <EyeOff size={15} />
                  ) : (
                    <Eye size={15} />
                  )}
        ...
        extraFooterContent={
          <div className="flex items-center gap-2 mr-auto">
            <input
              ...
              onChange={(e) =>
                setModelFormData({ ...modelFormData, synced: e.target.checked })
              }
              className="appearance-none -webkit-appearance-none w-[18px] h-[18px] border-[1.5px] border-zinc-700/50 rounded bg-[var(--bg-primary)] cursor-pointer relative transition-all flex-shrink-0 inline-flex items-center justify-center vertical-middle checked:bg-blue-500 checked:border-blue-500 checked:shadow-[0_0_20px_rgba(59,130,246,0.2)] hover:border-blue-500 focus:outline-none focus:shadow-[0_0_0_3px_rgba(59,130,246,0.2)] disabled:opacity-50 disabled:cursor-not-allowed"
            />
            <label
              htmlFor="model-synced"
              className="text-sm font-medium text-zinc-300 cursor-pointer"
            >
              {t.common.syncThisItem || "Sync this item"}
            </label>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          <input
            ...
            onChange={(e) =>
              setModelFormData({ ...modelFormData, enabled: e.target.checked })
            }
          />
          <label htmlFor="model-active" className="text-sm cursor-pointer">
            {t.ai.modelForm.active}
          </label>
        </div>
                  ...
                  {fetchedModels
                    .filter(
                      (m) =>
                        ...
                        m
                          .toLowerCase()
                          .includes(modelFormData.name.toLowerCase()),
                    )
                    .map((model) => (
                      <li
                        key={model}
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          <label
            ...
            className="block text-sm font-medium text-zinc-400 mb-1.5 "
          >
            {t.ai.modelForm.channel}
          </label>
          <CustomSelect
            id="model-channel"
            value={modelFormData.channelId || ""}
            onChange={(val) =>
              setModelFormData({ ...modelFormData, channelId: val })
            }
