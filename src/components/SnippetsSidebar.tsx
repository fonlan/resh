  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[var(--bg-tertiary)] [&::-webkit-scrollbar-thumb]:rounded-[3px]">
        {snippets.length === 0 ? (
          <p className="text-[12px] text-[var(--text-muted)] text-center mt-6 px-4 leading-relaxed">
            {t.snippetsTab.emptyState}
          </p>
        ) : (
          groupNames.map((groupName) => (
            <div key={groupName} className="mb-1">
              ...
              <div
                className={`pl-2 flex flex-col gap-1 overflow-hidden transition-all duration-200 ${!expandedGroups[groupName] ? "hidden" : ""}`}
              >
                {groupedSnippets[groupName].map((snippet) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <Play
                        size={10}
                        className="text-[var(--accent-success)] opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus:opacity-100"
                      />
                    </div>
